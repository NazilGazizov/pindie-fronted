...
export default function Profile() {
    ...
    //     item.users_permissions_users.includes(store.user);
    // });
    ...
    return (
        ...
        <div className={Styles['profile']}>
            <div className={Styles['basic__information']}>
                <img alt="avatar" src="/images/zdes.jpg" className={Styles['avatar']}/>
                <h2>{store.user.username}</h2>
                <p>{store.user.role.id === 1 ? 'Пользователь' : 'Аккаунт практикума'}</p>
            </div>
            <div className={Styles['full__information']}>
                <div className={Styles['title']}>
                    <p className={Styles['title__text']}>Информация</p>
                ...
                <div className={Styles['title']}>
                    <p className={Styles['title__text']}>Игры за которые вы проголосовали:</p>
                    <hr className={Styles['line']}/>
                </div>
                <div className={Styles['games']}>
                    <p>типа тут игры</p>
                    <p>upd в разработке</p>
                    <a href="https://clck.ru/39j9be" target="_blank">*помочь в разработке*</a>
                </div>
            </div>
        </div>
        </>) : preloaderVisible ? (
    ...
    :  (<section classNameName={Styles['not__found']}><ProfileNotFound/></section>)}
        </main>
    )
};
